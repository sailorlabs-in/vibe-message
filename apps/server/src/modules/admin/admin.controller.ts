...
export class AdminController {
  ...
  async getNotifications() {
    const notifications =
      await this.internalNotificationService.getAdminNotifications();
  ...
  async updateUserStatus(
    @Param("id") id: string,
    @Body() data: UpdateUserStatusRequest,
  ) {
  ...
  async updateUserRetentionPermission(
    @Param("id") id: string,
    @Body() data: { canManageRetention: boolean },
  ) {
    const user = await this.userService.updateUserRetentionPermission(
      parseInt(id, 10),
  ...
  async updateUserAppLimit(
    @Param("id") id: string,
    @Body() data: UpdateAppLimitRequest,
  ) {
    const user = await this.userService.updateUserAppLimit(
      parseInt(id, 10),
      data,
    );
    return { success: true, data: user };
  ...
  async warnUser(
    @Req() req: any,
    @Param("id") id: string,
    @Body() data: CreateWarningRequest,
  ) {
    const warning = await this.userService.createWarning(
      parseInt(id, 10),
      req.user.userId,
      data,
    );
  ...
  async deleteUser(@Req() req: any, @Param("id") id: string) {
    const result = await this.userService.deleteUserBySuperAdmin(
      parseInt(id, 10),
      req.user.userId,
    );
    return {
