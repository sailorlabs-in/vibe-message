...
import { UserResponse, UserStatus, UpdateUserStatusRequest, UpdateUserRoleRequest, UpdateAppLimitRequest, CreateWarningRequest } from '../../types';
...
export class UserService {
  ...
  async updateUserStatus(userId: number, data: UpdateUserStatusRequest): Promise<UserResponse> {
    ...
    try {
      if (data.status === 'APPROVED') {
        this.internalNotificationService.notifyUserApproved(userId, user.name).catch((err: any) => console.error(err));
      } else if (data.status === 'BANNED') {
        this.internalNotificationService.notifyUserBanned(userId).catch((err: any) => console.error(err));
      }
    } catch (e) {}
  ...
  async updateUserAppLimit(userId: number, data: UpdateAppLimitRequest): Promise<UserResponse> {
    ...
    try {
      this.internalNotificationService.notifyUserAppLimitChanged(userId, data.appLimit).catch((err: any) => console.error(err));
    } catch(e) {}

    return this.userToResponse(user);
  }

  async createWarning(userId: number, createdBy: number, data: CreateWarningRequest): Promise<Warning> {
    const user = await this.userRepository.findOne({ where: { id: userId } });
    if (!user) {
      throw new NotFoundException('User not found');
    }
    ...
    try {
      this.internalNotificationService.notifyUserWarned(userId, data.message).catch((err: any) => console.error(err));
    } catch(e) {}
  ...
  async updateUserProfile(userId: number, name?: string, email?: string): Promise<UserResponse> {
    if (email) {
      const emailCheck = await this.userRepository.findOne({ where: { email, id: Not(userId) } });
      if (emailCheck) {
  ...
  async updateUserRetentionPermission(userId: number, canManageRetention: boolean): Promise<UserResponse> {
    const user = await this.userRepository.findOne({ where: { id: userId } });
    if (!user) {
      throw new NotFoundException('User not found');
    }
  ...
  async deleteUserBySuperAdmin(targetUserId: number, superAdminId: number): Promise<{ deletedAppsCount: number }> {
    const superAdmin = await this.userRepository.findOne({ where: { id: superAdminId } });
    ...
    const appsCount = await this.appRepository.count({ where: { user_id: targetUserId } });
