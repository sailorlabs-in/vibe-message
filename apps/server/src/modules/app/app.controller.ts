...
export class AppController {
  ...
  async getAppNotifications(
    ...
    const notifications = await this.pushService.getAppNotifications(app.id, 100, isScheduled);
  ...
  async deleteNotification(
    ...
    await this.pushService.deleteNotification(app.id, parseInt(notificationId, 10));
    return {
      success: true,
  ...
  async getMembers(@Req() req: any, @Param('id') id: string) {
    const members = await this.appService.getAppMembers(id, req.user.userId, req.user.role);
