...
export class SystemController {
  ...
  async getSettings() {
    ...
    return {
      success: true,
      data: { default_retention_days: settings ? settings.default_retention_days : 14 }
    };
  }
  ...
  async updateSettings(@Body() body: any, @Res() res: Response) {
    ...
    if (typeof default_retention_days !== 'number' || default_retention_days < 1) {
      return res.status(HttpStatus.BAD_REQUEST).json({ success: false, message: 'Invalid retention days value.' });
    }

    let settings = await this.systemSettingsRepository.findOne({ where: { id: 1 } });
    if (!settings) {
      settings = this.systemSettingsRepository.create({ id: 1, default_retention_days });
    } else {
