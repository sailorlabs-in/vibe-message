...
export const Header: React.FC = () => {
  ...
  return (
    <>
      <div className="fixed top-0 left-0 w-full z-50 flex justify-center pt-4 px-4 sm:px-6 pointer-events-none">
        <header
          className={`pointer-events-auto bg-theme-bg-secondary/90 backdrop-blur-md border border-theme-border flex flex-col transition-all duration-500 ease-[cubic-bezier(0.25,1,0.5,1)] shadow-2xl shadow-theme-border/10 overflow-hidden ${
            isMenuOpen
              ? 'rounded-[2rem] w-full md:max-w-md max-h-[800px]'
              : 'rounded-[2rem] w-full max-w-4xl max-h-16'
          }`}
        >
          <div className="flex justify-between items-center h-16 w-full px-6 flex-shrink-0">
            <Link
              to="/"
              className="text-xl font-bold text-theme-primary-600 dark:text-theme-primary-400 flex items-center gap-2 flex-shrink-0"
            >
            ...
            <nav className="hidden md:flex items-center space-x-6 ml-auto">
              {user ? (
                ...
                <>
                  ...
                  <Link
                    to="/login"
                    className="text-sm px-4 py-2 bg-theme-primary-600 hover:bg-theme-primary-700 text-white rounded-full font-medium transition-all shadow-lg shadow-theme-primary-500/20 hover:scale-105 ml-2"
                  >
                    Login
                  </Link>
                </>
            ...
            <div className="flex items-center md:hidden">
              ...
              >
                <div className="flex flex-col items-center justify-center w-5 h-5 relative">
                  <span
                    className={`absolute block w-5 h-0.5 bg-current transition-all duration-300 origin-center ${isMenuOpen ? 'rotate-45' : '-translate-y-1.5'}`}
                  ></span>
                  <span
                    className={`absolute block w-5 h-0.5 bg-current transition-all duration-300 origin-center ${isMenuOpen ? 'opacity-0 scale-50' : 'opacity-100 scale-100'}`}
                  ></span>
                  <span
                    className={`absolute block w-5 h-0.5 bg-current transition-all duration-300 origin-center ${isMenuOpen ? '-rotate-45' : 'translate-y-1.5'}`}
                  ></span>
              ...
              {user ? (
                <>
                  ...
                  <button
                    ...
                    className="w-full text-center font-bold text-white bg-theme-error/90 hover:bg-theme-error px-4 py-3 mt-2 rounded-xl transition-colors shadow-lg shadow-theme-error/20"
                  >
                    Logout
                  </button>
                </>
      ...
      {/* Logout Confirmation — portalled above everything */}
      <ConfirmModal
