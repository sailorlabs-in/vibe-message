...
export const NotificationHistory: React.FC<NotificationHistoryProps> = ({
  ...
  const fetchNotifications = async () => {
    try {
      ...
      toast.error(
        isScheduled ? 'Failed to load scheduled messages' : 'Failed to load notifications history'
      );
    } finally {
      setLoading(false);
    }
        ...
        <table className="w-full text-left border-collapse">
          ...
          <tbody>
            {notifications.map((notif) => {
              let payloadObj: any = {};
              try {
                payloadObj = JSON.parse(notif.payload_json);
              } catch {
                //error
              }
                  ...
                  <tr className="border-b border-theme-border hover:bg-theme-bg-secondary/50 transition-colors">
                    <td className="p-4 align-top">
                      <div className="text-sm font-medium text-theme-text-primary whitespace-nowrap">
                        {new Date(
                          isScheduled && notif.scheduled_at ? notif.scheduled_at : notif.created_at
                        ).toLocaleDateString()}
                      </div>
                                      ...
                                      <tr key={log.id}>
                                        ...
                                        <td className="px-4 py-2 text-xs">
                                          ...
                                          {![
                                            ...
                                            'Firefox',
                                            'Edge/Windows',
                                          ].includes(log.device_token?.platform || '') && (
