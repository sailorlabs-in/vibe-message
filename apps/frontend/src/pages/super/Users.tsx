...
export const Users: React.FC = () => {
  ...
  const handleToggleRetentionPerm = async (user: User) => {
    ...
    if (updateUserRetentionPermission.fulfilled.match(result)) {
      toast.success(
        `Retention permission ${!user.can_manage_retention ? "granted" : "revoked"}`
      );
    } else {
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-theme-border bg-black/5 dark:bg-white/5">
                <th className="py-4 px-5 text-xs font-bold uppercase tracking-widest text-theme-text-muted">Name</th>
                <th className="py-4 px-5 text-xs font-bold uppercase tracking-widest text-theme-text-muted">Email</th>
                <th className="py-4 px-5 text-xs font-bold uppercase tracking-widest text-theme-text-muted">Status</th>
                <th className="py-4 px-5 text-xs font-bold uppercase tracking-widest text-theme-text-muted">Role</th>
                <th className="py-4 px-5 text-xs font-bold uppercase tracking-widest text-theme-text-muted">App Limit</th>
                <th className="py-4 px-5 text-xs font-bold uppercase tracking-widest text-theme-text-muted text-right">Actions</th>
              </tr>
            </thead>
            <tbody>
              {users.length === 0 ? (
                <tr>
                  <td colSpan={6} className="py-16 text-center text-theme-text-secondary font-medium">
                ...
                users.map((user, idx) => (
                  ...
                  >
                    ...
                    {/* Status badge */}
                    <td className="py-4 px-5">
                      <span className={`px-3 py-1 rounded-full text-xs font-bold inline-block ${statusColors[user.status] ?? ""}`}>
                        {user.status}
                      </span>
                    </td>
                    ...
                    {/* Role badge */}
                    <td className="py-4 px-5">
                      <span className={`flex items-center gap-1.5 w-fit px-3 py-1 rounded-full text-xs font-bold ${roleColors[user.role] ?? ""}`}>
            ...
            <form onSubmit={handleAppLimitSubmit} className="space-y-5">
              <div>
                <label className="block text-sm font-medium mb-2 text-theme-text-primary">
                  App Limit{" "}
                  <span className="text-theme-text-muted font-normal">(leave blank for unlimited)</span>
                </label>
                <input
                  type="number"
                  value={appLimit}
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          ...
          >
            ...
            <form onSubmit={handleWarningSubmit} className="space-y-5">
              ...
              />
              <div className="flex gap-3 pt-2">
                <button type="submit" className="btn-primary flex-1">Send Warning</button>
                <button type="button" onClick={closeModal} className="btn-secondary flex-1">Cancel</button>
              </div>
            </form>
          </motion.div>
